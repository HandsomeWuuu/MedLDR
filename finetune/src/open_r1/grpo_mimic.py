...
def main(data_args,script_args, training_args, model_args):
    ...
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(sys.stdout),
                  logging.FileHandler(script_args.log_file)],
    )
    ...
    # if 'llama' in model_args.model_name_or_path, Please select a token to use as `pad_token` `(tokenizer.pad_token = tokenizer.eos_token e.g.)`
    ...
    # dataset[train_split] = dataset[train_split].shuffle(seed=training_args.seed).select(range(10000))
...
if __name__ == "__main__":
    parser = TrlParser((DataArguments, GRPOScriptArguments, GRPOConfig, ModelConfig))
    data_args, script_args, training_args, model_args = parser.parse_args_and_config()
    
    if training_args.local_rank == 0:  # Only print on rank 0
        print("script_args: \n", script_args)
        print("training_args: \n", training_args)
        print("model_args: \n", model_args)
    ...
    main(data_args, script_args, training_args, model_args)
